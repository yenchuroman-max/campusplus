...
- Ключ API не работает:
  - `.env` не подгружается автоматически, задавайте переменные окружения явно перед запуском.
...
Важно: в текущем коде `.env` не подгружается автоматически (`python-dotenv` не используется). Нужны именно переменные окружения процесса.
